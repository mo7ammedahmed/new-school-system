...
Before approving release, run `bash scripts/verify-native-release.sh` from a native checkout or CI runner. The script refuses the known mounted workspace and runs `composer install`, fresh and refresh migration/seed validation against a disposable SQLite database, Wayfinder generation, `npm ci`, frontend lint and type checks, the production build, `composer lint:check`, `composer types:check`, `php artisan test`, and `php artisan app:release-check --strict`. Any failure is a no-go condition. The same gate is available through `.github/workflows/native-release.yml` for manual or `main`/`master` push execution.
...
6. Run `app:release-check --strict`.
