...
=======
    locale?: 'ar' | 'en';
    school?: { id: number; name: string };
    schools?: Array<{ id: number; name: string; students_count: number }>;
    metrics?: {
        students?: number;
        applications?: number;
        notices?: number;
    ...
    upcomingExamPeriods?: ExamPeriod[];
...
=======
    ...
    const managementLinks = schoolBase
        ? [
              {
                  href: `${schoolBase}/applications`,
                  icon: UsersRound,
                  ar: 'القبول والتسجيل',
                  en: 'Admissions',
              },
              {
                  ...
                  icon: GraduationCap,
                  ar: 'الأكاديميات والتسجيل',
                  en: 'Academics and enrollment',
              },
              {
                  href: `${schoolBase}/schedule/setup`,
                  icon: GraduationCap,
                  ar: 'إعداد الجدول والمواد',
                  en: 'Schedule setup and subjects',
              },
              {
                  href: `${schoolBase}/schedule/timetable`,
                  icon: CalendarCheck2,
                  ar: 'جدول المعلمين',
                  en: "Teachers' timetable",
              },
              {
                  href: `${schoolBase}/schedule/exams`,
                  icon: CalendarCheck2,
                  ar: 'جداول الاختبارات',
                  en: 'Exam schedules',
              },
              {
                  href: `${schoolBase}/pages`,
                  icon: FileText,
                  ar: 'صفحات الموقع العام',
                  en: 'Public website pages',
              },
              {
                  href: `${schoolBase}/notices`,
                  icon: Megaphone,
                  ar: 'إعلانات المدرسة',
                  en: 'School notices',
              },
              {
                  href: `${schoolBase}/finance`,
                  icon: WalletCards,
                  ar: 'المالية والفواتير',
                  en: 'Finance and invoices',
              },
              {
                  href: `${schoolBase}/reports/attendance`,
                  icon: BarChart3,
                  ar: 'تقارير الحضور',
                  en: 'Attendance reports',
              },
              {
                  href: `${schoolBase}/reports/finance`,
                  icon: CreditCard,
                  ar: 'تقارير المالية',
                  en: 'Finance reports',
              },
...
=======
                    <div className="flex flex-wrap items-center gap-3">
                        {schools.length > 1 ? (
                            <label className="flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-bold">
...
=======
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    <section className="rounded-[1.75rem] border border-[#dbe8df] bg-white p-6">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h2 className="mt-1 text-2xl font-black text-[#17342f]">
                                    {isArabic
                                        ? 'ما يُختبر اليوم'
                            ...
                            {schoolBase ? (
                                <Link
                                    href={`${schoolBase}/schedule/exams`}
                                    className="shrink-0 text-sm font-black text-[#0d5c4d]"
                                >
                        ...
                        {todayExams.length === 0 ? (
                            <p className="mt-4 text-sm text-[#789087]">
                                {isArabic
                                    ? 'لا يوجد اختبار منشور اليوم.'
                                    : 'No published exam today.'}
                            </p>
                        ) : (
                            <ol className="mt-4 space-y-3">
                                {todayExams.map((paper) => (
                                    <li
                                        key={paper.id}
                    ...
                    <section className="rounded-[1.75rem] border border-[#dbe8df] bg-white p-6">
                        <p className="text-sm font-bold text-[#789087]">
                            {isArabic ? 'فترات الاختبارات' : 'Exam periods'}
                        </p>
                        <h2 className="mt-1 text-2xl font-black text-[#17342f]">
                            {isArabic
                                ? 'القادم من الاختبارات'
                                : 'Upcoming exam periods'}
                        </h2>
                        {upcomingExamPeriods.length === 0 ? (
                            <p className="mt-4 text-sm text-[#789087]">
                                {isArabic
                                    ? 'لا توجد فترة اختبارات منشورة.'
                                    : 'No published exam period yet.'}
                            </p>
                        ) : (
                            <ol className="mt-4 space-y-3">
                                {upcomingExamPeriods.map((period) => (
                                    <li
                                        key={period.id}
                                        className="rounded-2xl border border-[#edf2ed] p-3"
                                    >
                                        <strong className="block text-sm font-black text-[#17342f]">
                                            {isArabic && period.title_ar
                                                ? period.title_ar
                                                : period.title}
                                        </strong>
                                        <small className="mt-1 block text-[#789087]">
                                        ...
                                        <small className="mt-1 block text-[#0d5c4d]">
                                            {isArabic
                                                ? `${period.papers_count} ورقة اختبار`
                                                : `${period.papers_count} papers`}
                                        </small>
                                    </li>
                                ))}
                            </ol>
                    ...
                    <section className="rounded-[1.75rem] border border-[#dbe8df] bg-white p-6">
                        <p className="text-sm font-bold text-[#789087]">
                            {isArabic ? 'حصصي اليوم' : 'My classes today'}
                        </p>
                        <h2 className="mt-1 text-2xl font-black text-[#17342f]">
                            {isArabic ? 'جدول اليوم' : "Today's timetable"}
                        </h2>
                        {todayClasses.length === 0 ? (
                            <p className="mt-4 text-sm text-[#789087]">
                                {isArabic
                                    ? 'لا توجد حصص منشورة لك اليوم.'
                                    : 'No published class assigned to you today.'}
                            </p>
                        ) : (
                            <ol className="mt-4 space-y-3">
                                {todayClasses.map((entry) => (
                                    ...
                                    >
                                        <span className="text-sm font-black text-[#17342f]">
                                            {subjectLabel(entry)}
                                        </span>
                                        <small className="text-[#789087]">
                                            {isArabic ? 'الحصة' : 'P'}{' '}
                                            {entry.period} ·{' '}
                                            {entry.section_name}
                                        </small>
                ...
                <div className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
                    <section className="rounded-[1.75rem] border border-[#dbe8df] bg-white p-6">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <Link
                                ...
                                className="text-sm font-black text-[#0d5c4d]"
                            >
                        ...
                        <div className="mt-6 space-y-3">
                            {[
                                isArabic
                                    ? [
                                          'طلبات قبول جديدة',
                                          '8 طلبات تنتظر المراجعة',
                                          schoolBase
                                              ? `${schoolBase}/applications`
                                              : '/contact',
                                      ]
                                    : [
                                          'New applications',
                                          '8 applications await review',
                                          schoolBase
                                              ? `${schoolBase}/applications`
                                              : '/contact',
                                      ],
                                isArabic
                                    ? [
                                          'فواتير متأخرة',
                                          '24 فاتورة تحتاج متابعة',
                                          schoolBase
                                              ? `${schoolBase}/reports/finance`
                                              : '/pricing',
                                      ]
                                    : [
                                          'Outstanding invoices',
                                          '24 invoices need follow-up',
                                          schoolBase
                                              ? `${schoolBase}/reports/finance`
                                              : '/pricing',
                                      ],
                                ...
                                <Link
                                    key={title}
                                    href={href}
                                    className="flex items-center gap-4 rounded-2xl border border-[#edf2ed] p-4 transition hover:border-[#0d5c4d] hover:bg-[#fbfdf9]"
                                >
                                    <CheckCircle2
                                        className="shrink-0 text-[#c56a3b]"
                                        size={21}
                                        aria-hidden="true"
                                    />
                                    ...
                                    <ArrowUpLeft
                                        className="ms-auto shrink-0 text-[#789087]"
                                        size={17}
                                        aria-hidden="true"
                                    />
                                </Link>
                    ...
                    <section className="rounded-[1.75rem] border border-[#dbe8df] bg-white p-6 lg:col-span-2">
                        <p className="text-sm font-bold text-[#789087]">
                            {isArabic ? 'مركز التحكم' : 'Control center'}
                        </p>
                        <h2 className="mt-1 text-2xl font-black text-[#17342f]">
                            {isArabic
                                ? 'إدارة كل عمليات المدرسة'
                                : 'Manage every school operation'}
                        </h2>
                        {managementLinks.length > 0 ? (
                            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                {managementLinks.map(
                                    ({ href, icon: Icon, ar, en }) => (
                                        <Link
                                            key={href}
                                            href={href}
                                            className="flex min-h-24 flex-col justify-between gap-3 rounded-2xl border border-[#e5eee7] bg-[#fbfdf9] p-4 font-black text-[#28544a] transition hover:-translate-y-0.5 hover:border-[#0d5c4d] hover:shadow-sm"
                                        >
                                            <Icon
                                                size={20}
                                                aria-hidden={true}
                            ...
                            <p className="mt-5 rounded-2xl bg-[#fff8ed] p-4 text-sm font-bold text-[#8a5a22]">
                                {isArabic
                                    ? 'لم يتم ربط مدرسة بحسابك بعد.'
                                    : 'No school is linked to your account yet.'}
                            </p>
                        )}
                    </section>

                    <section className="rounded-[1.75rem] border border-[#dbe8df] bg-[#f7f8f4] p-6 lg:col-span-2">
                        <p className="text-sm font-bold text-[#789087]">
                        ...
                        <div className="mt-6 space-y-3">
                            {quickLinks.map(({ href, icon: Icon, ar, en }) => (
                                <Link
                                    key={href}
                                    href={href}
                                    className="flex items-center gap-3 rounded-2xl bg-white p-4 font-black text-[#28544a] transition hover:-translate-y-0.5 hover:shadow-sm"
                                >
